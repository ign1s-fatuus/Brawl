...
int main()
{
    ...
    printw("Generate World ... PASSED\n");
    getch();
    
    newLevel = allocateMemory(newLevel);
    printw("Allocate Memory ... PASSED\n");
    getch();
    
    newLevel = buildColorPalette(newLevel);
    printw("Build Color Palette ... PASSED\n");
    getch();
    
    newLevel = setUpPlayer(newLevel);
    printw("Player Setup ... PASSED\n");
    getch();

    newLevel = generateWorldHeightMap(newLevel);
    printw("Generate World Map ... PASSED\n");
    getch();

    newLevel = generateLevel(newLevel, 1);
    printw("Generate Level ... PASSED\n");
    getch();

    newLevel = generateMiniMap(newLevel);
    printw("Generate MinMap ... PASSED\n");
    getch();
...
Level * allocateMemory(Level * newLevel)
{
    ...
    // WorldTiles array
    newLevel->worldTiles = (WorldTiles **)malloc(sizeof(WorldTiles) * newLevel->worldHeight);
    for (y = 0; y < newLevel->worldHeight; y++)
    {
        newLevel->worldTiles[y] = (WorldTiles *)malloc(sizeof(WorldTiles) * newLevel->worldWidth);
    }
